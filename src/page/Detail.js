              ...
              <p>
				...
				<ul className="select_area">
					...
					>
						<span>서비스</span>
						<strong>
							{servicesText.length > 0 ? servicesText.join(', ') : "서비스를 선택해주세요."}
						</strong>
					</li>
					<li
						onClick={() => {
					...
					>
						<span>날짜</span>
						<strong>
							{dates.length > 0 && dates.length < 4 ? dates.sort().join(', ') : dates.length >= 4 ? `4일 이상 선택(총 ${dates.length}일)` :"날짜를 선택해주세요."}
						</strong>
					</li>
				</ul>
				<div className="buttons">
					<button type="button">문의하기</button>
					<button type="button" onClick={requestReservation}>예약하기</button>
				</div>
			</ReservationFunctions>
		</SitterDetailPage>
	);
