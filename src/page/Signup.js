	...
	return (
			<Form onSubmit={handleSubmit}>
				...
				<label className="inner required">
					...
					<Input _width="100%" _height="44px" _placeholder="4~10자리(특수문자, 숫자, 영어 포함)" _type="password" id="pw" _name={"password"} onChange={pwCheck} required />
					{ values.password && <Message className={`${isPw ? "success" : "error"}`}>{pwMessage}</Message>}
				</label>
				<label className="inner required">
					<p className="tit">비밀번호 확인</p>
					<Input _width="100%" _height="44px" _placeholder="4~10자리(특수문자, 숫자, 영어 포함)" _type="password" id="pw2" onChange={isSamePw} required />
					{ values.password && <Message className={`${isPw2 ? "success" : "error"}`}>{pw2Message}</Message>}
				</label>
				<label className="inner required">
					<p className="tit">핸드폰번호</p>
					<Input _width="100%" _height="44px" _placeholder="'-' 없이 입력해주세요" _type="text" _value={phoneCurrent} _name={"phoneNumber"} onChange={phoneRegexr} required />
					{ values.phoneNumber && <Message className={`${isPhone ? "success" : "error"}`}>{phoneMessage}</Message>}
				</label>
				<label className="inner required">
