...
const Petprofile = () => {
	...
	useEffect(() => {
		(petSuccessGet || petSuccessDelete) && setValues(petprofileData.data.petprofile)
				...
				values.length > 0 ? ( values.map((el, idx) => {
					...
					return (
						<div key={petId} className="petprofileItem">
							<div>
								<span className="bgImg" style={{backgroundImage: `url(${petImage})`}}></span>
								<span>{petName}<span>{petType}</span></span>
							</div>
							<div>
								<Link to={`/mypage/${petId}/petprofileform`} state={{data: el}}>수정</Link>
								<button onClick={() => {delect(petId)}}>삭제</button>
							</div>
						</div>
					)
				})) : (
				 <Link to={{pathname:`/mypage/petprofileform`}}><button>반려동물을 등록하세요</button></Link>
				)
			}
		</PetprofileInner>
	);
